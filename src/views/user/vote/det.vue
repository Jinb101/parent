    ...
    <template #fixed>
      <v-f v-model="open" :text="childdet.full_name" :index="610">
        <div class="vote_h3">
          ...
          <van-divider content-position="left">活动规则</van-divider>
          <div class="content">
            <p>
              每个微信号每天可投票点赞{{ det.number_votes }}次，每次{{
                det.score_per_vote
...
export default {
  ...
  components: { vS, vP, vF, vA, vV, vC, vCL },
  computed: {
    votetitle() {
  ...
  methods: {
    ...
    onprogressvideo(e) {
      ...
      if (+this.childdet.broadcasting === 0) {
        this.$api.http(
          "a2023052306",
          {
            voters_id: this.childdet.voters_id,
    ...
    toimageshow(e, flag) {
      let pic = (e.picture || [])[0] || "";
      let pic2 = ((e.child || {}).base_map || {}).base_map || "";
      if (flag) {
        return pic2;
      }
      return pic || pic2 || "";
