      ...
      <template v-if="+i.sub === 2">
        <div class="v_box">
          <template v-if="isadd">
            ...
            />
            <p>{{ i.label }}</p>
            <template v-if="write.data">
              <div class="hand_img">
                <van-image
                  :src="write.data"
                  @click="appShowImage(write.data)"
                ></van-image>
              </div>
            </template>
            <div class="btn">
              <van-button
                ...
                plain
                @click="open = true"
                >点击{{ write.data ? "重置" : "" }}签名</van-button
              >
            </div>
          </template>
          <template v-else>
...
export default {
  ...
  methods: {
    onsave(ee) {
      this.$demo.es6().imageCompress(ee, {}, (res) => {
        this.$api.upload({ type: "img", imgs: res.small }, (r) => {
          if (r.code === 200) {
            this.write = r.data || {};
          }
          this.open = false;
    ...
    onsubmit() {
      ...
      this.$set(this.item[p], "hand", this.write.data);
      this.$set(this.item[p], "path", this.write.halfway);
      this.$nextTick(() => {
        let os = {
          record_date: this.toTime(this.ext.record_date),
          content: JSON.stringify(this.item),
          parent_signature: this.write.halfway,
