...
import {
  Icon, Lazyload, Image, Popup, Picker, ShareSheet, ImagePreview, Tabbar, TabbarItem, NoticeBar, Field,
  Grid, GridItem, NavBar, Empty, Swipe, SwipeItem, Loading, Collapse, CollapseItem, Button, Radio, RadioGroup,
  DatetimePicker, Checkbox, Tab, Tabs, Rate, Stepper, ActionSheet, Cell, Badge, Tag, Search, Divider, CellGroup,
  CheckboxGroup, PullRefresh, List, Switch, Popover, Progress, Circle, Calendar
} from 'vant'
...
if ($js.env) {
  Image.methods.onError = function (e) {
    try { e.target.src = $js.path + 'static/image/theme/t4.png' } catch (err) { }
  }
}
// vant
Vue.use(Icon).use(Lazyload).use(Image).use(Popup).use(Picker).use(ShareSheet).use(ImagePreview).use(Tabbar)
  .use(TabbarItem).use(NoticeBar).use(Field).use(Grid).use(GridItem).use(NavBar).use(Empty).use(Swipe).use(SwipeItem)
  .use(Loading).use(Collapse).use(CollapseItem).use(Button).use(Radio).use(RadioGroup).use(DatetimePicker).use(Divider)
  .use(Checkbox).use(Tab).use(Tabs).use(Rate).use(Stepper).use(ActionSheet).use(Cell).use(Badge).use(Tag).use(Search)
  .use(CellGroup).use(CheckboxGroup).use(PullRefresh).use(List).use(Switch).use(Popover).use(Progress).use(Circle)
  .use(Calendar)
...
Vue.prototype.$js = $js
Vue.prototype.$vx = $vx
Vue.prototype.$theme = window.demo.xj().theme(window.demo.getUrl().theme || 'orange')
