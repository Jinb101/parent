...
var demo_calculate = function () {
  ...
  var check = function (a = 0, b = 0) {
    var ac = t._is_obj(a).code, bc = t._is_obj(b).code
    var p = [0]
    var z = [0]
    if (ac === 4) {
      p = p.concat(a)
  ...
  return {
    ...
    */
    sortSum: function (arr, fn, flag) {
      ...
      function s() {
        flag ? t.array(a).sort().sort() : t.array(a).sort().reverse()
        let v = a[1]
        if (v !== undefined) {
          let ns = a[0] + v
          o.push(ns)
    ...
    */
    adj: function (a, tx, flag) {
      ...
      switch (flag) {
        case 1:
          t.array(arr).sort().reverse()
          break
        case 2:
          t.array(arr).sort().sort()
          break
        default:
      }
...
var demo_vx = function (cls) {
  ...
  let geoOpen = function (obj, fn) {
    ...
    wx.openLocation({
      ...
      scale: obj.scale || 1, // 地图缩放级别,整形值,范围从1~28。默认为最大
      ...
      demo_vx().image(res => {
        res.length && res.forEach(id => {
          demo_vx().imageGet({ id }, fileId => {
            if (fileId.code) {
              demo.es6().imageCompress(fileId.data.url, { width: 1920, height: 1080 }, file => {
                img.push(file)
                if (img.length === res.length) {
                  resolve(img.filter(s => { return !!s }))
                }
              }, err => {
                console.log(err, 'err')
              })
...
var demo_validate = function () {
  let rex = {
    ...
    strength: /^(?=.*\d)(?=.*[a-z]).{6,18}$/, // 6-18位 英文或数字
    zh: /^[\u4e00-\u9fa5]{0,}$/, // 中文
    w: /^\w+$/, // 数字、26个英文字母或下划线
    decimal: /^\d+\.\d+$/, // 小数
    name: /^[\u4E00-\u9FA5]{2,4}$/, // 中文名字
  ...
  // 必须是包含字母和数字的组合 upper为true 时 必须包含 大写。min,max 长度
  let is_password = (val, min, max, upper) => {
    min = min || 8
    max = max || 16
    let up = upper ? '(?=.*[A-Z])' : ''
    let abs = new RegExp("^(?=.*\\d)(?=.*[a-z])" + up + ".{" + min + "," + max + "}$")
    return abs.test(val)
  }
...
var demo_xj = function () {
  ...
  let theme = (name) => {
    ...
    let linear = 'linear-gradient(45deg,' + os.color[0] + ',' + (os.color[2] || os.color[1]) + ')'
    let ret = {
      ...os,
...
var siteConfig = () => {
  ...
  let errorinfo = (options = {}) => {
    ...
    let setMsg = (data) => {
      if (JSON.stringify(arr).length / 1024 > opt.size) {
        ...
        arr.push({ ...data, use_author: config, use_timeout: Date.now(), use_type: '家长端' })
      }
    }
  ...
  return {
    errorinfo, menulevel, icon, tel, text
  }
}
